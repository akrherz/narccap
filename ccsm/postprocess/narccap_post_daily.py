...
BASETS = mx.DateTime.DateTime(1968,1,1,0)
...
def create_file(vname, ts0, ts1):
    ...
    nc.title = 'ISU MM5 model output prepared for NARCCAP present-day climate using HADCM3'
    ...
    nc.realization = '1'
    nc.experiment_id = 'present-day climate using HADCM3'
    ...
    nc2 = netCDF3.Dataset('data/MMOUTP_DOMAIN1_0001.nc', 'r')
    # write lat
    ...
    for yr in range(ts0.year, ts1.year):
        for mo in range(1,13):
            for dy in range(1,31):
                ...
                if offset2 < offset1:
                    offset2 = -1
                nc2 = netCDF3.Dataset('data/NCOUT_DOMAIN1_%04i.nc' % (i,), 'r')
                ...
                if dy in [10,20,30]: # Uh oh, need two files!
                    nc2 = netCDF3.Dataset('data/NCOUT_DOMAIN1_%04i.nc' % (i+1,), 'r')
