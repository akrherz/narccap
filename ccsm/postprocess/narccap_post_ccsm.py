...
def create_file(VNAME, ts0, ts1):
    ...
    """
    ncfn = '../final/%s_%s_ccsm_%s03.nc' % (VNAME, META['prefix'],
                                            ts0.strftime("%Y%m%d"))
    if PLEVEL is not None:
        ncfn = '../final/%s_%s_ccsm_p%03i_%s.nc' % (VNAME, META['prefix'],
                                                    PLEVEL,
    ...
    if common.VARS[VNAME]['interval'] == HOURLY3:
        ...
        tm[:] = offset + np.arange(0.25, tsteps, 1.)
        # write tmb
        if 'cell_methods' in common.VARS[VNAME]:
            tmb[:, 0] = offset + np.arange(0.25, tsteps, 1.)
            tmb[:, 1] = offset + np.arange(1.25, tsteps+1., 1.)
...
def compute1d(VNAME, fp, ts0, ts1):
    ...
    for i in range(1, 1500):
        fp2 = '%s/%s_DOMAIN1_%04i.nc' % (DATADIR, common.VARS[VNAME]['source'],
                                         i)
    ...
    while now < ts1:
        ...
        data = common.VARS[VNAME]['npfunc'](
            nc2.variables[common.VARS[VNAME]['ncsource']][offset1:offset2,
                                                          15:-15,
                                                          15:-15], axis=0)
        nc2.close()

        if offset2 == tsteps:  # Need to step ahead and get next file
            fp2 = '%s/%s_DOMAIN1_%04i.nc' % (DATADIR,
                                             common.VARS[VNAME]['source'], i)
            nc2 = netCDF4.Dataset(fp2, 'r')
            data2 = nc2.variables[common.VARS[VNAME]['ncsource']][:1,
                                                                  15:-15,
                                                                  15:-15]
            nc2.close()
        ...
        print(('%s %s %02i %02i Avg: %.3f'
               ) % (now.strftime("%Y%m%d"), fp2, offset1, offset2,
                    np.average(data)))
...
def compute3h(VNAME, fp, ts0, ts1):
    ...
    while v < total:
        ...
        if VNAME == 'mrfso':
            data = ((nc2.variables['soil_m_1'][:, 15:-15, 15:-15] -
                     nc2.variables['soil_w_1'][:, 15:-15, 15:-15]) * 0.10 +
                    (nc2.variables['soil_m_2'][:, 15:-15, 15:-15] -
                     nc2.variables['soil_w_2'][:, 15:-15, 15:-15]) * 0.30 +
                    (nc2.variables['soil_m_3'][:, 15:-15, 15:-15] -
                     nc2.variables['soil_w_3'][:, 15:-15, 15:-15]) * 0.60 +
                    (nc2.variables['soil_m_4'][:, 15:-15, 15:-15] -
                     nc2.variables['soil_w_4'][:, 15:-15, 15:-15]) *
                    1.00) * 1000.0

        elif VNAME == 'mrso':
            data = ((nc2.variables['soil_m_1'][:, 15:-15, 15:-15]) * 0.10 +
            ...
            data = nc2.variables['rain_con'][:,15:-15,15:-15] + nc2.variables['rain_non'][:,15:-15,15:-15]
        ...
        print(("i=%4i tsteps=%2i %5i %5i/%5i %.5f"
               ) % (i, tsteps, v, v2, total,
