...
def create_file(VNAME, ts0, ts1):
    ...
    """
    fp = '../final/%s_%s_hadcm3_%s03.nc' % (VNAME, META['prefix'],
                                            ts0.strftime("%Y%m%d"))
    if PLEVEL is not None:
        fp = ('../final/%s_%s_hadcm3_p%03i_%s.nc'
              ) % (VNAME, META['prefix'], PLEVEL, ts0.strftime("%Y%m%d%H"))
    ...
    nc.source = ('MM5(2002): atmosphere: MM5v3.6.3 non-hydrostatic; '
                 ...
                 'Vertical Levels: 24; Deepsoil: %s') % (DSON,)
    ...
    for i in range(offset, offset2):
        ...
        if VNAME == 'mrfso':
            data = ((nc2.variables['soil_m_1'][:, 15:-15, 15:-15] -
                    ...
                    0.10 + (nc2.variables['soil_m_2'][:, 15:-15, 15:-15] -
                            nc2.variables['soil_w_2'][:, 15:-15, 15:-15]) *
                    0.30 + (nc2.variables['soil_m_3'][:, 15:-15, 15:-15] -
                            nc2.variables['soil_w_3'][:, 15:-15, 15:-15]) *
                    0.60 + (nc2.variables['soil_m_4'][:, 15:-15, 15:-15] -
            ...
            proj = (nc2.variables[ncs][:, plev, :-1, :-1] +
                    nc2.variables[ncs][:, plev, 1:, 1:] +
                    nc2.variables[ncs][:, plev, :-1, 1:] +
                    nc2.variables[ncs][:, plev, 1:, :-1]) / 4.0
